...
const path_1 = tslib_1.__importDefault(__webpack_require__(2));
...
const task_1 = tslib_1.__importDefault(__webpack_require__(35));
...
(function (ConfirmationSelection) {
    ConfirmationSelection[ConfirmationSelection["deny"] = 1] = "deny";
    ConfirmationSelection[ConfirmationSelection["disable"] = 2] = "disable";
    ConfirmationSelection[ConfirmationSelection["allow"] = 3] = "allow";
    ConfirmationSelection[ConfirmationSelection["alwaysAllow"] = 4] = "alwaysAllow";
...
(function (ConfirmExecution) {
    ConfirmExecution.type = new coc_nvim_1.RequestType('eslint/confirmESLintExecution');
})(ConfirmExecution || (ConfirmExecution = {}));
var ShowOutputChannel;
(function (ShowOutputChannel) {
    ShowOutputChannel.type = new coc_nvim_1.NotificationType0('eslint/showOutputChannel');
})(ShowOutputChannel || (ShowOutputChannel = {}));
const exitCalled = new coc_nvim_1.NotificationType('eslint/exitCalled');
async function pickFolder(folders, placeHolder) {
    if (folders.length === 1) {
        return Promise.resolve(folders[0]);
    }
    const selected = await coc_nvim_1.window.showQuickpick(folders.map((folder) => { return folder.name; }), placeHolder);
    if (selected === -1) {
        return undefined;
    }
...
function createDefaultConfiguration() {
    ...
    if (!folders) {
        coc_nvim_1.window.showErrorMessage('An ESLint configuration can only be generated if VS Code is opened on a workspace folder.');
        return;
    }
    const noConfigFolders = folders.filter(folder => {
        ...
        for (const configFile of configFiles) {
            if (fs_1.default.existsSync(path_1.default.join(coc_nvim_1.Uri.parse(folder.uri).fsPath, configFile))) {
                return false;
            }
        }
        return true;
    ...
    if (noConfigFolders.length === 0) {
        if (folders.length === 1) {
            coc_nvim_1.window.showInformationMessage('The workspace already contains an ESLint configuration file.');
        }
        else {
            coc_nvim_1.window.showInformationMessage('All workspace folders already contain an ESLint configuration file.');
        }
        return;
    }
    pickFolder(noConfigFolders, 'Select a workspace folder to generate a ESLint configuration for').then(async (folder) => {
        ...
        const folderRootPath = coc_nvim_1.Uri.parse(folder.uri).fsPath;
        const terminal = await coc_nvim_1.workspace.createTerminal({
            name: `ESLint init`,
            cwd: folderRootPath
        });
        const eslintCommand = await utils_1.findEslint(folderRootPath);
        terminal.sendText(`${eslintCommand} --init`);
        terminal.show();
    });
}
let onActivateCommands;
const probeFailed = new Set();
function computeValidate(textDocument) {
    const config = coc_nvim_1.workspace.getConfiguration('eslint', textDocument.uri);
...
let eslintExecutionState;
const eslintAlwaysAllowExecutionKey = 'eslintAlwaysAllowExecution';
let eslintAlwaysAllowExecutionState = false;
const sessionState = new Map();
const disabledLibraries = new Set();
const resource2ResourceInfo = new Map();
let globalStatus;
let lastExecutionInfo;
const libraryPath2ExecutionInfo = new Map();
const workspaceFolder2ExecutionInfos = new Map();
function updateExecutionInfo(params, result) {
    ...
    if (value === undefined) {
        value = {
            ...
            diagnostics: coc_nvim_1.languages.createDiagnosticCollection()
        };
...
function getExecutionInfo(doc, strict) {
    ...
    if (!strict) {
        const folder = coc_nvim_1.workspace.getWorkspaceFolder(doc.uri);
        if (folder !== undefined) {
...
async function askForLibraryConfirmation(client, context, params, update) {
    ...
    const libraryUri = coc_nvim_1.Uri.file(params.libraryPath);
    const folder = coc_nvim_1.workspace.getWorkspaceFolder(libraryUri.toString());
    let message;
    if (folder !== undefined) {
    ...
    const item = await coc_nvim_1.window.showInformationMessage(message, ...messageItems);
    // Dialog got canceled.
    if (item === undefined) {
        return;
    }
    ...
    update && update();
    client && client.sendNotification(vscode_languageserver_protocol_1.DidChangeConfigurationNotification.type, { settings: {} });
}
async function resetLibraryConfirmations(client, context, update) {
    ...
    const selectedIdx = await coc_nvim_1.window.showQuickpick(items.map(o => o.label), 'Clear library confirmations');
    if (selectedIdx == -1) {
        return;
    }
...
function activate(context) {
    ...
    function configurationChanged() {
        ...
        for (const textDocument of coc_nvim_1.workspace.textDocuments) {
            if (computeValidate(textDocument) !== Validate.off) {
                openListener.dispose();
                configurationListener.dispose();
                activated = true;
    ...
    let activated = false;
    const openListener = coc_nvim_1.workspace.onDidOpenTextDocument(didOpenTextDocument);
    const configurationListener = coc_nvim_1.workspace.onDidChangeConfiguration(configurationChanged);
    const notValidating = async () => {
        ...
        const enabled = coc_nvim_1.workspace.getConfiguration('eslint', doc ? doc.uri : undefined).get('enable', true);
        if (!enabled) {
            coc_nvim_1.window.showInformationMessage(`ESLint is not running because the deprecated setting 'eslint.enable' is set to false. Remove the setting and use the extension disablement feature.`);
        }
        else {
            coc_nvim_1.window.showInformationMessage('ESLint is not running. By default only TypeScript and JavaScript files are validated. If you want to validate other file types please specify them in the \'eslint.probe\' setting.');
        }
    };
    onActivateCommands = [
        coc_nvim_1.commands.registerCommand('eslint.executeAutofix', notValidating),
        coc_nvim_1.commands.registerCommand('eslint.showOutputChannel', notValidating),
        coc_nvim_1.commands.registerCommand('eslint.manageLibraryExecution', notValidating),
        coc_nvim_1.commands.registerCommand('eslint.resetLibraryExecution', () => {
            resetLibraryConfirmations(undefined, context, undefined);
        })
    ];
    context.subscriptions.push(coc_nvim_1.commands.registerCommand('eslint.createConfig', createDefaultConfiguration));
    context.subscriptions.push(new task_1.default());
    configurationChanged();
}
exports.activate = activate;
var CodeActionsOnSave;
(function (CodeActionsOnSave) {
    ...
    CodeActionsOnSave.isExplicitlyDisabled = isExplicitlyDisabled;
    ...
    CodeActionsOnSave.getSourceFixAll = getSourceFixAll;
    ...
    CodeActionsOnSave.getSourceFixAllESLint = getSourceFixAllESLint;
    ...
    CodeActionsOnSave.setSourceFixAllESLint = setSourceFixAllESLint;
...
function realActivate(context) {
    ...
    function updateStatusBar(status, isValidated) {
        ...
        const alwaysShow = coc_nvim_1.workspace.getConfiguration('eslint').get('alwaysShowStatus', false);
    ...
    async function updateStatusBarAndDiagnostics() {
        ...
        function handleEditor(doc) {
            ...
            if (info.result === ConfirmExecutionResult.confirmationPending && ((_a = info.editorErrorUri) === null || _a === void 0 ? void 0 : _a.toString()) !== uri.toString()) {
                const range = (_b = doc.getWordRangeAtPosition(coc_nvim_1.Position.create(0, 0))) !== null && _b !== void 0 ? _b : coc_nvim_1.Range.create(0, 0, 0, 0);
                const diagnostic = coc_nvim_1.Diagnostic.create(range, 'ESLint is disabled since its execution has not been approved or denied yet. Use :CocCommand eslint.showOutputChannel to open the approval dialog.', coc_nvim_1.DiagnosticSeverity.Warning);
                ...
                info.editorErrorUri = coc_nvim_1.Uri.parse(errorUri);
                if (info.codeActionProvider !== undefined) {
                    info.codeActionProvider.dispose();
                }
                info.codeActionProvider = coc_nvim_1.languages.registerCodeActionProvider([{ pattern: coc_nvim_1.Uri.parse(errorUri).fsPath }], {
        ...
        function findApplicableStatus(editor) {
            ...
            if (editor !== undefined) {
                ...
                const workspaceFolder = coc_nvim_1.workspace.getWorkspaceFolder(editor.uri);
    ...
    function readCodeActionsOnSaveSetting(document) {
        let result = undefined;
        const languageConfig = coc_nvim_1.workspace.getConfiguration(undefined, document.uri).get(`[${document.languageId}]`);
        function isEnabled(value) {
            ...
            if (Array.isArray(value)) {
                ...
                return (_a = value['source.fixAll.eslint']) !== null && _a !== void 0 ? _a : value['source.fixAll'];
            }
        }
        if (languageConfig !== undefined) {
            const codeActionsOnSave = languageConfig === null || languageConfig === void 0 ? void 0 : languageConfig['editor.codeActionsOnSave'];
            if (codeActionsOnSave !== undefined) {
                result = isEnabled(codeActionsOnSave);
            }
        }
        if (result === undefined) {
            const codeActionsOnSave = coc_nvim_1.workspace.getConfiguration('editor', document.uri).get('codeActionsOnSave');
            if (codeActionsOnSave !== undefined) {
                result = isEnabled(codeActionsOnSave);
            }
        }
    ...
    const eslintConfig = coc_nvim_1.workspace.getConfiguration('eslint');
    ...
    const serverOptions = {
        run: { module: serverModule, transport: coc_nvim_1.TransportKind.ipc, runtime, options: { cwd: coc_nvim_1.workspace.cwd, env } },
        debug: { module: serverModule, transport: coc_nvim_1.TransportKind.ipc, runtime, options: { execArgv: ['--nolazy', '--inspect=6011'], cwd: process.cwd(), env } }
    };
    ...
    const confirmationSemaphore = new utils_1.Semaphore(1);
    const supportedQuickFixKinds = new Set([vscode_languageserver_protocol_1.CodeActionKind.Source, vscode_languageserver_protocol_1.CodeActionKind.SourceFixAll, `${vscode_languageserver_protocol_1.CodeActionKind.SourceFixAll}.eslint`, vscode_languageserver_protocol_1.CodeActionKind.QuickFix]);
    const clientOptions = {
        ...
        middleware: {
            didOpen: (document, next) => {
                if (coc_nvim_1.workspace.match([packageJsonFilter], document) || coc_nvim_1.workspace.match([configFileFilter], document) || computeValidate(document) !== Validate.off) {
                    next(document);
            ...
            provideCodeActions: (document, range, context, token, next) => {
                ...
                const newContext = Object.assign({}, context, { diagnostics: eslintDiagnostics });
            ...
            workspace: {
                ...
                configuration: async (params, _token, _next) => {
                    ...
                    for (const item of params.items) {
                        ...
                        const config = coc_nvim_1.workspace.getConfiguration('eslint', resource);
                        const workspaceFolder = coc_nvim_1.workspace.getWorkspaceFolder(resource);
                        const settings = {
                        ...
                        const workingDirectories = config.get('workingDirectories', undefined);
                        if (Array.isArray(workingDirectories)) {
                            let workingDirectory = undefined;
                            const workspaceFolderPath = workspaceFolder && coc_nvim_1.Uri.parse(workspaceFolder.uri).scheme === 'file' ? coc_nvim_1.Uri.parse(workspaceFolder.uri).fsPath : undefined;
                                ...
                                if (directory !== undefined || pattern !== undefined) {
                                    ...
                                    if (filePath !== undefined) {
                                        if (directory !== undefined) {
                                            directory = utils_1.toOSPath(directory);
                                            if (!path_1.default.isAbsolute(directory) && workspaceFolderPath !== undefined) {
                                                directory = path_1.default.join(workspaceFolderPath, directory);
                                            }
                                            if (directory.charAt(directory.length - 1) !== path_1.default.sep) {
                                                directory = directory + path_1.default.sep;
                                            }
                                            ...
                                            if (!path_1.default.posix.isAbsolute(pattern) && workspaceFolderPath !== undefined) {
                                                pattern = path_1.default.posix.join(utils_1.toPosixPath(workspaceFolderPath), pattern);
                                            }
                                            if (pattern.charAt(pattern.length - 1) !== path_1.default.posix.sep) {
                                                pattern = pattern + path_1.default.posix.sep;
                                            }
                                            const regExp = utils_1.convert2RegExp(pattern);
                                            if (regExp !== undefined) {
    ...
    let client;
    try {
        client = new coc_nvim_1.LanguageClient('ESLint', serverOptions, clientOptions);
    }
    catch (err) {
        coc_nvim_1.window.showErrorMessage(`The ESLint extension couldn't be started. See the ESLint output channel for details.`);
        return;
    }
    ...
    coc_nvim_1.workspace.onDidChangeConfiguration(() => {
        ...
        for (const textDocument of syncedDocuments.values()) {
            if (computeValidate(textDocument) === Validate.off) {
                try {
                    const provider = client.getFeature(vscode_languageserver_protocol_1.DidCloseTextDocumentNotification.method).getProvider(textDocument);
                    provider === null || provider === void 0 ? void 0 : provider.send(textDocument);
                }
                catch (err) {
        ...
        for (const textDocument of coc_nvim_1.workspace.textDocuments) {
            if (!syncedDocuments.has(textDocument.uri.toString()) && computeValidate(textDocument) !== Validate.off) {
                try {
                    const provider = client.getFeature(vscode_languageserver_protocol_1.DidOpenTextDocumentNotification.method).getProvider(textDocument);
                    provider === null || provider === void 0 ? void 0 : provider.send(textDocument);
                }
                catch (err) {
                    // A feature currently throws if no provider can be found. So for now we catch the exception.
    ...
    client.onDidChangeState((event) => {
        if (event.newState === coc_nvim_1.State.Starting) {
            client.info('ESLint server is starting');
            serverRunning = undefined;
        }
        else if (event.newState === coc_nvim_1.State.Running) {
            client.info(running);
            serverRunning = true;
        }
        else {
        ...
        updateStatusBar((globalStatus !== null && globalStatus !== void 0 ? globalStatus : serverRunning === false) ? Status.error : Status.ok, true);
    });
    client.onReady().then(() => {
        ...
        client.onNotification(exitCalled, (params) => {
            ...
            coc_nvim_1.window.showErrorMessage(`ESLint server shut down itself. See 'ESLint' output channel for details.`, { title: 'Open Output', id: 1 }).then((value) => {
        ...
        client.onRequest(NoConfigRequest.type, (params) => {
            const uri = coc_nvim_1.Uri.parse(params.document.uri);
            const workspaceFolder = coc_nvim_1.workspace.getWorkspaceFolder(params.document.uri);
        ...
        client.onRequest(NoESLintLibraryRequest.type, (params) => {
            ...
            const uri = coc_nvim_1.Uri.parse(params.source.uri);
            const workspaceFolder = coc_nvim_1.workspace.getWorkspaceFolder(uri.toString());
            const packageManager = coc_nvim_1.workspace.getConfiguration('eslint', uri.toString()).get('packageManager', 'npm');
            const localInstall = {
            ...
            if (workspaceFolder) {
                ...
                if (!state.workspaces[workspaceFolder.uri.toString()]) {
                    ...
                    coc_nvim_1.window.showInformationMessage(`Failed to load the ESLint library for the document ${uri.fsPath}. See the output for more information.`, outputItem).then((item) => {
                        if (item && item.id === 1) {
                            client.outputChannel.show(true);
                        }
                    });
        ...
        client.onRequest(OpenESLintDocRequest.type, (params) => {
            coc_nvim_1.commands.executeCommand('vscode.open', coc_nvim_1.Uri.parse(params.url));
            return {};
        });
        client.onRequest(ProbeFailedRequest.type, (params) => {
            ...
            for (const document of coc_nvim_1.workspace.textDocuments) {
                if (document.uri.toString() === params.textDocument.uri) {
                    (_a = closeFeature.getProvider(document)) === null || _a === void 0 ? void 0 : _a.send(document);
                }
            }
        });
        client.onRequest(ConfirmExecution.type, async (params) => {
            return confirmationSemaphore.lock(async () => {
                try {
                    ...
                    result = result !== null && result !== void 0 ? result : ConfirmExecutionResult.confirmationPending;
                    ...
                    if (executionInfo === undefined) {
                        ...
                        const workspaceFolder = coc_nvim_1.workspace.getWorkspaceFolder(params.uri);
                        if (workspaceFolder !== undefined) {
    ...
    context.subscriptions.push(client.start(), coc_nvim_1.events.on('BufEnter', () => {
        updateStatusBarAndDiagnostics();
    }), coc_nvim_1.workspace.registerTextDocumentContentProvider('eslint-error', {
    ...
    }), coc_nvim_1.commands.registerCommand('eslint.executeAutofix', async () => {
        ...
        client.sendRequest(vscode_languageserver_protocol_1.ExecuteCommandRequest.type, params).then(undefined, () => {
            coc_nvim_1.window.showErrorMessage('Failed to apply ESLint fixes to the document. Please consider opening an issue with steps to reproduce.');
        });
    }), coc_nvim_1.commands.registerCommand('eslint.showOutputChannel', async () => {
        let doc = await coc_nvim_1.workspace.document;
    ...
    }), coc_nvim_1.commands.registerCommand('eslint.manageLibraryExecution', async (params) => {
        if (params !== undefined) {
            ...
            if (info !== undefined) {
                ...
                coc_nvim_1.window.showInformationMessage(doc && doc.attached
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.Semaphore = exports.toPosixPath = exports.toOSPath = exports.convert2RegExp = exports.findEslint = void 0;
const tslib_1 = __webpack_require__(1);
const fs_1 = tslib_1.__importDefault(__webpack_require__(3));
const path_1 = tslib_1.__importDefault(__webpack_require__(2));
...
async function findEslint(rootPath) {
    ...
    if (platform === 'win32' && await exists(path_1.default.join(rootPath, 'node_modules', '.bin', 'eslint.cmd'))) {
        return path_1.default.join('.', 'node_modules', '.bin', 'eslint.cmd');
    }
    else if ((platform === 'linux' || platform === 'darwin') && await exists(path_1.default.join(rootPath, 'node_modules', '.bin', 'eslint'))) {
...
(function (NodeType) {
    ...
    NodeType["brace"] = "brace";
    NodeType["bracket"] = "bracket";
    NodeType["questionMark"] = "questionMark";
    NodeType["star"] = "star";
    NodeType["globStar"] = "globStar";
})(NodeType || (NodeType = {}));
...
exports.convert2RegExp = convert2RegExp;
...
class EslintTask {
    constructor() {
        this.disposables = [];
        this.statusItem = coc_nvim_1.window.createStatusBarItem(1, { progress: true });
        let task = this.task = coc_nvim_1.workspace.createTask('ESLINT');
        this.disposables.push(coc_nvim_1.commands.registerCommand(EslintTask.id, async () => {
        ...
        task.onExit(code => {
            if (code != 0) {
                coc_nvim_1.window.showMessage(`Eslint found issues`, 'warning');
            }
            this.onStop();
        });
        task.onStdout(lines => {
        ...
        task.onStderr(lines => {
            coc_nvim_1.window.showMessage(`TSC error: ` + lines.join('\n'), 'error');
        });
    ...
    async getOptions() {
        let root = coc_nvim_1.Uri.parse(coc_nvim_1.workspace.workspaceFolder.uri).fsPath;
        let cmd = await utils_1.findEslint(root);
        let config = coc_nvim_1.workspace.getConfiguration('eslint');
