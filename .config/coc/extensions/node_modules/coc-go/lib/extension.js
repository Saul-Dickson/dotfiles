...
async function registerGopls(context) {
    ...
    const command = config.goplsPath || await tools_1.goBinPath(binaries_1.GOPLS);
    if (!await tools_1.commandExists(command) && !await tools_1.installGoBin(binaries_1.GOPLS)) {
        return;
    }
    const serverOptions = {
        command,
